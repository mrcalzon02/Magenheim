...
def material(name,stem,rough_default,emission=False):
    ...
    alb=nt.nodes.new('ShaderNodeTexImage'); alb.image=image(stem,'albedo'); nt.links.new(uv.outputs['Generated'],alb.inputs['Vector']); nt.links.new(alb.outputs['Color'],bs.inputs['Base Color'])
    rough=nt.nodes.new('ShaderNodeTexImage'); rough.image=image(stem,'roughness'); nt.links.new(uv.outputs['Generated'],rough.inputs['Vector']); nt.links.new(rough.outputs['Color'],bs.inputs['Roughness']); bs.inputs['Roughness'].default_value=rough_default
    normal=nt.nodes.new('ShaderNodeTexImage'); normal.image=image(stem,'normal'); nm=nt.nodes.new('ShaderNodeNormalMap'); nt.links.new(uv.outputs['Generated'],normal.inputs['Vector']); nt.links.new(normal.outputs['Color'],nm.inputs['Color']); nt.links.new(nm.outputs['Normal'],bs.inputs['Normal'])
    if emission:
        em=nt.nodes.new('ShaderNodeTexImage'); em.image=image(stem,'emission'); nt.links.new(uv.outputs['Generated'],em.inputs['Vector'])
