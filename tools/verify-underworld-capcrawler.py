...
if sc.get('magenheim_fidelity')!='production-creature-r5': raise RuntimeError('Expected production-creature-r5')
...
# Pads, not cylindrical shins, are now the authoritative ground-contact anatomy.
...
print(f'VERIFIED Capcrawler r5 source gate: meshes={len(meshes)} triangles={triangles} bones={len(bones)} materials={len(mats)} actions={len(REQ_ACTIONS)} extent={extent} pads=8 claws=8',flush=True)
