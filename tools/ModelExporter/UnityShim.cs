...
public class Transform : Component
{
    ...
    {
        var scaled = Vector3.Scale(point, localScale);
        var rotated = localRotation * scaled;
        var local = rotated + localPosition;
...
public struct Quaternion
{
    ...
    public static Quaternion FromToRotation(Vector3 from, Vector3 to)
    {
        ...
        if (dot < -.999999f)
        {
            var axis = Vector3.Cross(a, Vector3.right);
            if (axis.sqrMagnitude < 1e-6f) axis = Vector3.Cross(a, Vector3.up);
            axis = axis.normalized;
            return new Quaternion(NQuat.CreateFromAxisAngle(new NVec3(axis.x, axis.y, axis.z), MathF.PI));
        }
        var c = Vector3.Cross(a, b).normalized;
        return new Quaternion(NQuat.CreateFromAxisAngle(new NVec3(c.x, c.y, c.z), MathF.Acos(dot)));
    }
    public static Quaternion operator *(Quaternion a, Quaternion b) => new(NQuat.Normalize(a.value * b.value));
    public static Vector3 operator *(Quaternion q, Vector3 v)
    {
        var r = NVec3.Transform(new NVec3(v.x, v.y, v.z), q.value);
...
public struct Color
{
    ...
    public static Color yellow => new(1f, .9215686f, .01568628f, 1f);
    public static Color cyan => new(0f,1f,1f,1f);
    public static Color magenta => new(1f,0f,1f,1f);
    public static Color grey => new(.5f,.5f,.5f,1f);
...
{
    public static string ToHtmlStringRGB(Color c) => $"{ToByte(c.r):X2}{ToByte(c.g):X2}{ToByte(c.b):X2}";
    private static int ToByte(float v) => (int)MathF.Round(Math.Clamp(v, 0f, 1f) * 255f);
}
...
public class Renderer : Component { public bool enabled = true; public Material? sharedMaterial; public Material[] sharedMaterials = Array.Empty<Material>(); }
public class MeshRenderer : Renderer { }
public class ParticleSystemRenderer : Renderer { }
public class LODGroup : Component { public bool enabled = true; }
public class Light : Component { public Color color; public float intensity; }
...
public class Material : Object
{
    ...
    public Material() { }
    public Material(Material source) { name = source.name; color = source.color; mainTexture = source.mainTexture; renderQueue = source.renderQueue; }
...
public class Texture2D : Texture
{
    ...
    public static Texture2D whiteTexture { get; } = new(1,1,TextureFormat.RGBA32,false);
...
{
    public static Sprite Create(Texture2D texture, Rect rect, Vector2 pivot, float pixelsPerUnit) => new();
}
...
public class BoxCollider : Collider { public Vector3 center; public Vector3 size; }
public class CapsuleCollider : Collider { public Vector3 center; public float radius; public float height; public int direction; }
public class MeshCollider : Collider { public Mesh? sharedMesh; public bool convex; }

public struct GradientColorKey { public Color color; public float time; public GradientColorKey(Color color,float time){this.color=color;this.time=time;} }
public struct GradientAlphaKey { public float alpha,time; public GradientAlphaKey(float alpha,float time){this.alpha=alpha;this.time=time;} }
public class Gradient { public void SetKeys(GradientColorKey[] colors, GradientAlphaKey[] alphas) { } }

public class ParticleSystem : Component
{
    public ColorOverLifetimeModule colorOverLifetime => new();
    public MainModule main => new();
    public readonly struct ColorOverLifetimeModule { public bool enabled { set { } } public MinMaxGradient color { set { } } }
    public readonly struct MainModule { public MinMaxGradient startColor { set { } } }
