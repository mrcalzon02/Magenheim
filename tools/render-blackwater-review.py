"""Repeatable B1b source review: front, side, three-quarter and silhouette, four models/page.

Run tools/blender.ps1 render-blackwater-review [page number ...]. Sources remain unwired.
...
ids = list(author.MODELS)
args = sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else []
pages = [int(a)-1 for a in args] if args else range(4)
...
for page in pages:
    ...
    for row, model_id in enumerate(ids[page*4:page*4+4]):
        with bpy.data.libraries.load(str(author.SOURCE / (model_id+'.blend')), link=False) as (src,dst):
    ...
    scene.render.filepath=str(OUT/f'blackwater-review-{page+1:02}.png')
