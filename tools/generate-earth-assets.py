...
def geode():
    ...
    for r in range(1,rings):
        ...
        for s in range(sides):
            t=2*math.pi*s/sides
            bump=1+.07*math.sin(3*t+2*p)+.05*math.cos(5*t-p)
            points.append((.21*math.sin(p)*math.cos(t)*bump,
                           .155*math.cos(p)*bump,
                           .18*math.sin(p)*math.sin(t)*bump))
    top, bottom=len(points),len(points)+1
    points.extend([(0,.156,0),(.012,-.15,0)])
    for s in range(sides):
        n=(s+1)%sides
        faces.extend([(top,s,n),(bottom,(rings-2)*sides+n,(rings-2)*sides+s)])
    for r in range(rings-2):
        for s in range(sides):
            a=r*sides+s; b=r*sides+(s+1)%sides; c=a+sides; d=b+sides
...
def crystal(tier):
    ...
    for ox,oy,oz,r,h in shards:
        ...
        for y,rad,twist in [(-h*.4,r*.72,0),(h*.23,r,0),(h*.43,r*.69,.04)]:
            for s in range(sides):
                a=s*math.pi/3+twist
                points.append((ox+rad*math.cos(a),oy+y,oz+rad*math.sin(a)))
        points.extend([(ox+r*.12,oy+h*.73,oz),(ox,oy-h*.49,oz)])
        for s in range(sides):
            n=(s+1)%sides
            faces.extend([(start+19,start+n,start+s),(start+12+s,start+12+n,start+18)])
            for ring in range(2):
                a=start+ring*6+s; b=start+ring*6+n
                faces.extend([(a,b,a+6),(b,b+6,a+6)])
        for _ in range(36):
            bases=[(117,76,35),(158,103,42),(198,143,62),(224,180,88),(238,207,133),(165,111,44)]
            base=bases[tier]
            shift=rng.randint(-23,19)
            colors.append(tuple(max(0,min(255,c+shift)) for c in base))
    return points,faces,colors
...
def emit(name, raw, outward=False):
    ...
    atlas=Image.new('RGB',(512,512)); d=ImageDraw.Draw(atlas)
    vertices,uvs,indices=[],[],[]
    columns=max(20, math.ceil(math.sqrt(len(faces))))
    tile=512//columns
    for i,(f,color) in enumerate(zip(faces,colors)):
        x=(i%columns)*tile; y=(i//columns)*tile
        d.rectangle((x,y,x+tile-1,y+tile-1),fill=color)
        d.polygon([(x+2,y+2),(x+tile-3,y+2),(x+3,y+8)],fill=tuple(min(255,c+10) for c in color))
        d.polygon([(x+2,y+tile-3),(x+tile-3,y+tile-3),(x+tile-3,y+tile-7)],fill=tuple(max(0,c-8) for c in color))
        for j,(u,v) in zip(f,[(3,3),(tile-4,3),(tile//2,tile-4)]):
            vertices.append(points[j]); uvs.append(((x+u)/512,1-(y+v)/512)); indices.append(len(indices))
    data={'vertices':vertices,'uv':uvs,'triangles':indices}
    (OUT/f'{name}.mesh.json').write_text(json.dumps(data,separators=(',',':'))+'\n')
    atlas.save(OUT/f'{name}.png')
    obj=['# Original Magenheim Earth asset; meters; Y up',f'mtllib {name}.mtl',f'usemtl {name}']
    obj += ['v '+' '.join(f'{x:.6f}' for x in p) for p in vertices]
    obj += ['vt '+' '.join(f'{x:.6f}' for x in p) for p in uvs]
    obj += [f'f {i+1}/{i+1} {i+2}/{i+2} {i+3}/{i+3}' for i in range(0,len(indices),3)]
...
def render(points,faces,colors):
    right=normal((1,0,-1)); up=normal((-.45,1,-.45)); view=normal(cross(right,up))
    project=lambda p:(dot(p,right),dot(p,up),dot(p,view))
    projected=[project(p) for p in points]
    lo=[min(p[k] for p in projected) for k in range(2)]
    hi=[max(p[k] for p in projected) for k in range(2)]
    scale=380/max(hi[k]-lo[k] for k in range(2)); center=[(hi[k]+lo[k])/2 for k in range(2)]
    img=Image.new('RGBA',(512,512)); d=ImageDraw.Draw(img)
    light=normal((-.4,1,1))
    for i in sorted(range(len(faces)),key=lambda i:sum(projected[j][2] for j in faces[i])):
        ...
        shade=.55+.45*max(0,dot(n,light))
        c=tuple(int(v*shade) for v in colors[i])+(255,)
        screen=[(256+(projected[j][0]-center[0])*scale,256-(projected[j][1]-center[1])*scale) for j in f]
        d.polygon(screen,fill=c)
    return img

def main():
    names=['geode','rough','simple','crystal','advanced','master','shards']
    icons=[]
