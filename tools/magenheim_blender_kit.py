...
def bake_atlas(parts, atlas, paint, size=ATLAS):
    """Bake every part's painted material into one shared atlas, then reduce each material to
    Principled plus the atlas, pad the gutters and filter down to the shipped size. A large model
    (a 12m tree) may ship the full bake instead: `size` = BAKE_SIZE skips the filter."""
...
def spike(points, width, thickness, samples=10, flat=False, up=None):
    """A tapering flame lock, ribbon or crest along a curve, ending in a point.

    `up` fixes the sweep's reference axis. A curve that bends back on itself (a fern frond curling
    down) can defeat the automatic choice below mid-path and fold the ribbon inside-out; the normal
    of the plane the curve bends in never can."""
...
def spec_painter(surfaces, ao_distance, edge_radius):
    """A paint(mat) for bake_atlas driven by a table of surface specs, keyed by mat['surface'].

    Spec keys: low/high -- mottled body ramp; scale -- noise scale; veins (colour, width, scale) --
    lines drawn along warped cell edges (cracks, seams, veins); stretch (x, y, z) -- optional
    coordinate scaling so the pattern runs as grain along an axis; edge (colour, gain); occlusion."""
    def paint(mat):
        spec = surfaces[mat['surface']]
        tree = mat.node_tree
        coords = tree.nodes.new('ShaderNodeTexCoord').outputs['Object']
        if 'stretch' in spec:
            coords = _stretched(tree, coords, spec['stretch'])
        ...
        finish(tree, base, srgb(*edge_colour), edge_gain, spec['occlusion'], ao_distance=ao_distance, edge_radius=edge_radius)
    return paint
