...
internal sealed class UnderworldMapTabRuntime : MonoBehaviour
{
    ...
    private bool EnsureUnderworldState(Minimap map)
    {
        ...
        UnderworldMinimapGameApi.GenerateWorldMap(map);
    ...
    private static void BindTextures(Minimap map, MapLayerState state)
    {
        ...
        UnderworldMinimapGameApi.SetForestMaskTexture(map, state.ForestMaskTexture);
        UnderworldMinimapGameApi.SetHeightTexture(map, state.HeightTexture);
        UnderworldMinimapGameApi.SetFogTexture(map, state.FogTexture);
    ...
    private sealed class MapLayerState
    {
        ...
        internal static MapLayerState CaptureCurrent(Minimap map, byte[] data, bool ownsTextures) => new()
        {
            ...
            ForestMaskTexture = UnderworldMinimapGameApi.GetForestMaskTexture(map),
            HeightTexture = UnderworldMinimapGameApi.GetHeightTexture(map),
            FogTexture = UnderworldMinimapGameApi.GetFogTexture(map),
            MapData = data,
        ...
        internal static MapLayerState CreateOwned(Minimap map)
        {
            var state = new MapLayerState
            {
                ...
                ForestMaskTexture = CloneTextureShape(UnderworldMinimapGameApi.GetForestMaskTexture(map), map.m_textureSize),
                HeightTexture = CloneTextureShape(UnderworldMinimapGameApi.GetHeightTexture(map), map.m_textureSize),
                FogTexture = CloneTextureShape(UnderworldMinimapGameApi.GetFogTexture(map), map.m_textureSize),
                OwnsTextures = true,
            };
            return state;
        }
