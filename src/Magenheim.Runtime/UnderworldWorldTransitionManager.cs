...
internal sealed class UnderworldWorldTransitionManager
{
    ...
    private void PrepareInstanceLifecycle(UnderworldWorldIdentity identity,UnderworldTransitionIntent active){if(active.TargetLayer==UnderworldLayer.Underworld)_instanceLifecycle.EnsureAdmitted(identity);}
    private void RehydrateStableLifecycle(UnderworldPlayerLayerState state,UnderworldWorldIdentity identity){if(state.CurrentLayer==UnderworldLayer.Surface)return;if(state.CurrentLayer!=UnderworldLayer.Underworld)throw new InvalidOperationException($"Cannot rehydrate unsupported stable Underworld layer '{state.CurrentLayer}'.");_instanceLifecycle.EnsureAdmitted(identity);_instanceLifecycle.EnsureActive(identity);}
