...
internal sealed class UnderworldRuntimeServices
{
    ...
    private UnderworldRuntimeServices(UnderworldSpatialDomainDefinition spatialDomain,UnderworldTransitionStateStore stateStore,UnderworldWorldPairManifestStore worldPairStore,ValheimPhysicalUnderworldWorldContextController worldContext,ValheimUnderworldTransitionPlacementHost placementHost,StoredUnderworldTransitionHost transitionHost,UnderworldWorldTransitionManager transitionManager,UnderworldTransitionRecoveryRuntime recoveryRuntime)
    ...
    {
        if(!UnderworldRuntimeIdentityResolver.TryResolveLocalSession(WorldPairStore,out identity,out layer,out playerId,out diagnostic)||identity is null)return false;if(layer==UnderworldLayer.Surface)WorldPairStore.EnsureManifest(identity);return true;
    }
