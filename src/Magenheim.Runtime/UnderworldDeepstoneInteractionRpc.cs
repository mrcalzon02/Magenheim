...
internal static class UnderworldDeepstoneInteractionRpc
{
    ...
    {
        if (stone is null || user is null || !stone.IsBound)
        {
            diagnostic = "Deepstone interaction is unavailable.";
            return false;
        }
        if (Vector3.Distance(user.transform.position, stone.transform.position) > 5f)
        {
            diagnostic = "Move closer to the Deepstone.";
            return false;
        }
        if (ZNet.instance is null)
        {
            diagnostic = "World network authority is unavailable.";
            return false;
        }
        if (ZNet.instance.IsServer())
            return TryActivateServer(stone, user, out diagnostic);
        if (_rpc is null || _authority is null || !_authority.IsClientMutationAuthorized)
        {
            diagnostic = "Server gameplay authority has not admitted Deepstone mutation.";
            return false;
        }
        var generation = _authority.ClientSessionGeneration;
        if (generation <= 0L)
        {
            diagnostic = "No active Magenheim authority session exists.";
            return false;
        }
        var package = new ZPackage();
        package.Write(RequestMessage);
        package.Write(generation);
        package.Write(stone.DeepstoneId);
    ...
    private static IEnumerator ReceiveServerMessage(long sender, ZPackage package)
    {
        try
        {
            ...
            var deepstoneId = package.ReadString();
            if (_authority is null || _rpc is null) yield break;
            var generation = _authority.GetPeerSessionGeneration(sender);
            if (generation <= 0L || generation != suppliedGeneration || !_authority.IsPeerMutationAuthorized(sender))
            {
                SendResponse(sender, generation, false, "Deepstone request belongs to an unauthorized or stale Magenheim session.");
                yield break;
            }
            if (!TryResolvePeerPlayer(sender, out var player, out var diagnostic) ||
                !TryResolveNearbyStone(player, deepstoneId, out var stone, out diagnostic))
            {
                SendResponse(sender, generation, false, diagnostic);
                yield break;
            }
            var applied = TryActivateServer(stone, player, out diagnostic);
            SendResponse(sender, generation, applied, diagnostic);
        }
        catch (Exception exception)
        {
            _log?.LogError($"Failed to process Deepstone interaction RPC from peer {sender}: {exception}");
        }
        yield break;
    }
    ...
    private static IEnumerator ReceiveClientMessage(long sender, ZPackage package)
    {
        try
        {
            ...
            if (_authority is null || sender != RuntimeGameApi.ServerPeerId ||
                generation <= 0L || generation != _authority.ClientSessionGeneration) yield break;
        ...
        {
            _log?.LogError($"Failed to process Deepstone interaction response: {exception}");
        }
        yield break;
    }
    ...
    private static bool TryActivateServer(UnderworldDeepstoneRuntime stone, Humanoid user, out string diagnostic)
    {
        ...
        {
            diagnostic = "Deepstone mutation requires server authority.";
            return false;
        }
        var boss = UnderworldDeepstoneRuntimeAuthority.BossForStone(stone.DeepstoneId);
        var inventory = user.GetInventory();
        var trophyItems = inventory.GetAllItems().Where(item => item.m_dropPrefab &&
            string.Equals(item.m_dropPrefab.name, boss.TrophyPrefabName, StringComparison.Ordinal)).ToArray();
        ...
        var remaining = plan.ConsumeTrophyCount;
        foreach (var item in trophyItems)
        {
            ...
            {
                diagnostic = "The required trophy changed before the server could consume it.";
                return false;
            }
            remaining -= consume;
        }
        if (remaining != 0)
        {
            diagnostic = "The server could not complete the authorized trophy consumption.";
            return false;
        }
        if (!stone.PersistAuthorizedActivation(plan.ResultingState.TrophyMounted, plan.ResultingState.BoonUnlocked))
        {
            ...
            _log?.LogError($"Deepstone '{stone.DeepstoneId}' consumed an authorized trophy but persistence failed.");
            return false;
        }
        diagnostic = plan.Diagnostic;
        return true;
    ...
    {
        stone = UnityEngine.Object.FindObjectsOfType<UnderworldDeepstoneRuntime>()
            .FirstOrDefault(value => value.IsBound && string.Equals(value.DeepstoneId, deepstoneId, StringComparison.Ordinal) &&
                                     Vector3.Distance(player.transform.position, value.transform.position) <= 5f)!;
    ...
    private static void SendResponse(long peerId, long generation, bool applied, string diagnostic)
    {
        ...
        var package = new ZPackage();
        package.Write(ResponseMessage); package.Write(generation); package.Write(applied); package.Write(diagnostic ?? string.Empty);
