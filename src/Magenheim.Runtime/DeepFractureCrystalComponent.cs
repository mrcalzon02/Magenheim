...
namespace Magenheim.Runtime
{
    ...
    internal sealed class DeepFractureCrystalComponent : MonoBehaviour, IDestructible
    {
        ...
        private void ApplyDestroyedState(){if(_renderers!=null)foreach(var renderer in _renderers)if(renderer!=null)renderer.enabled=false;if(_colliders!=null)foreach(var collider in _colliders)if(collider!=null)collider.enabled=false;if(_owner==null)return;switch(_function){case DeepFractureCrystalFunction.EnvironmentalControl:_owner.m_runSpeed=Mathf.Min(_owner.m_runSpeed,.75f);_owner.m_walkSpeed=Mathf.Min(_owner.m_walkSpeed,.35f);break;case DeepFractureCrystalFunction.Armor:_owner.m_health=Mathf.Min(_owner.m_health,_owner.GetMaxHealth()*.8f);break;case DeepFractureCrystalFunction.Regeneration:_owner.m_health=Mathf.Min(_owner.m_health,_owner.GetMaxHealth()*.9f);break;case DeepFractureCrystalFunction.Resistance:_owner.m_runSpeed=Mathf.Min(_owner.m_runSpeed,2.0f);break;case DeepFractureCrystalFunction.WorldCore:_owner.m_runSpeed=Mathf.Min(_owner.m_runSpeed,1.25f);_owner.m_walkSpeed=Mathf.Min(_owner.m_walkSpeed,.6f);break;}}
        internal bool IsDeathAnchorIntact(){if(_function!=DeepFractureCrystalFunction.DeathAnchor||_view==null||!_view.IsValid())return false;var zdo=_view.GetZDO();return zdo!=null&&!IsDestroyed(zdo);}
    }
}
