...
internal static class NowhereKingVisuals
{
    internal static void Apply(GameObject prefab)
    {
        ...
        var crown=ModelAssets.Load(prefab,"broken-crown",hideOriginal:false);crown.name="NowhereKing_BrokenCrown";crown.transform.localPosition=new Vector3(0f,2.05f,0f);crown.transform.localScale=new Vector3(.48f,.12f,.48f);var light=crown.AddComponent<Light>();light.color=new Color(.32f,0f,.02f);light.range=3.5f;light.intensity=1.15f;
    }
}
