...
internal sealed class NowhereKingRegistrar : IDisposable
{
    ...
    private void RegisterContent()
    {
        ...
        try
        {
            ...
            _registered = true;
            _log.LogInfo($"Registered unique boss prefab '{PrefabName}'.");
        }
...
internal static class NowhereKingVisuals
{
    internal static void Apply(GameObject prefab)
    {
        ...
        foreach (var renderer in renderers)
        {
            ...
            var material = new Material(source) { name = "Magenheim_NowhereKing_Armor" };
            if (material.HasProperty("_Color")) material.SetColor("_Color", new Color(0.018f, 0.02f, 0.027f, 1f));
        ...
        crown.name = "NowhereKing_BrokenCrown";
        crown.transform.SetParent(prefab.transform, false);
        crown.transform.localPosition = new Vector3(0f, 2.05f, 0f);
        crown.transform.localScale = new Vector3(.48f, .12f, .48f);
