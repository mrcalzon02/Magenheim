...
internal sealed class UnderworldWorldCenterRegistrar : IDisposable
{
    ...
    private void OnVanillaLocationsAvailable()
    {
        ...
        try
        {
            ...
            _registered=true;_log.LogInfo($"Registered Underworld center '{LocationName}' with standing stones and adjacent return Deep Gate.");
        }
    ...
    private static void BuildStandingStones(Transform parent)
    {
        ...
        const int count=8;const float radius=12f;
        for(var i=0;i<count;i++){var angle=i*Mathf.PI*2f/count;AddMesh(root.transform,$"StandingStone_{i+1:00}",box,new Vector3(Mathf.Cos(angle)*radius,2.8f,Mathf.Sin(angle)*radius),Quaternion.Euler(i%2==0?-3f:4f,-angle*Mathf.Rad2Deg+90f,i%3-1),new Vector3(2.1f,7.4f+(i%3)*0.7f,1.25f),material,parent.gameObject.layer);}
