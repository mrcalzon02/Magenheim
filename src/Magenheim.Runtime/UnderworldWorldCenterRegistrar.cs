...
internal sealed class UnderworldWorldCenterRegistrar : IDisposable
{
    ...
    private static readonly string[] DeepstoneNames =
    {
        "StoneOfBloom",
        "StoneOfDeepTide",
        "StoneOfCinder",
    ...
    private void OnVanillaLocationsAvailable()
    {
        ...
        try
        {
            ...
            var container=ZoneManager.Instance.CreateLocationContainer(LocationName)??throw new InvalidOperationException($"Jotunn could not create Underworld world-center container '{LocationName}'.");
            ...
            var custom=new CustomLocation(container,fixReference:false,config);if(!ZoneManager.Instance.AddCustomLocation(custom))throw new InvalidOperationException($"Jotunn refused Underworld world-center registration for '{LocationName}'.");
            _registered=true;_log.LogInfo($"Registered Underworld center '{LocationName}' with the six-stone Deepstone Conclave, Descent Monolith and adjacent return Deep Gate.");
        }
        catch(Exception exception){_log.LogError($"Underworld world-center registration failed: {exception}");throw;}
        finally{Dispose();}
    }

    private static void BuildStandingStones(Transform parent)
    {
        var root=new GameObject(StandingStonesName);root.transform.SetParent(parent,false);var material=CreateStoneMaterial();var box=RuntimeMeshPrimitives.Box("magenheim.underworld.standing-stone");var dais=RuntimeMeshPrimitives.Cylinder(32,"magenheim.underworld.center-dais");
        const float radius=12f;
        for(var i=0;i<DeepstoneNames.Length;i++)
        {
            var angle=i*Mathf.PI*2f/DeepstoneNames.Length;
            AddMesh(root.transform,DeepstoneNames[i],box,new Vector3(Mathf.Cos(angle)*radius,2.8f,Mathf.Sin(angle)*radius),Quaternion.Euler(i%2==0?-3f:4f,-angle*Mathf.Rad2Deg+90f,i%3-1),new Vector3(2.1f,7.4f+(i%3)*0.7f,1.25f),material,parent.gameObject.layer);
        }
