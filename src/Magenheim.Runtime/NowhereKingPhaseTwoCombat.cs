...
{
    private const float Upper=0.70f,Lower=0.35f;
    private Character _king; private ZNetView _view; private Vector3 _center; private bool _configured; private float _next; private int _sequence;
    ...
    private void MoveLegal(Vector3 requested){var local=requested-_center;local.x=Mathf.Clamp(local.x,-24f,24f);local.z=Mathf.Clamp(local.z,-28f,28f);_king.SetPos(_center+local);var body=GetComponent<Rigidbody>();if(body!=null)body.velocity=Vector3.zero;}
