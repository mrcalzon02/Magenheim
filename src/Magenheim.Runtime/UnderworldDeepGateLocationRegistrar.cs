...
/// <summary>
/// Additive surface-world placement for dormant Deep Gates. The location exists before unlock;
/// activation is intentionally delegated to the authoritative Nowhere-King/transition runtime.
/// </summary>
internal sealed class UnderworldDeepGateLocationRegistrar : IDisposable
{
    ...
    private void OnVanillaLocationsAvailable()
    {
        ...
        try
        {
            ...
            {
                _registered = true;
                _log.LogWarning($"Deep Gate site identity '{LocationName}' is already occupied; existing content was left untouched.");
                return;
            }

            var gatePrefab = PrefabManager.Instance.GetPrefab(UnderworldDeepGateRegistrar.PrefabName)
                ?? throw new InvalidOperationException($"Deep Gate prefab '{UnderworldDeepGateRegistrar.PrefabName}' was not registered before location composition.");

            var container = ZoneManager.Instance.CreateLocationContainer(LocationName)
                ?? throw new InvalidOperationException($"Jotunn could not create Deep Gate location container '{LocationName}'.");
            ...
            if (!ZoneManager.Instance.AddCustomLocation(custom))
                throw new InvalidOperationException($"Jotunn refused additive Deep Gate location registration for '{LocationName}'.");

            _registered = true;
            _log.LogInfo($"Registered persistent surface Deep Gate sites '{LocationName}' across eligible Valheim biomes.");
        }
