...
internal static class GeneratedSurfaceTextures
{
    ...
    private static int AppendProjectedVertex(
        ...
        Vector3 vertex,
        Vector3 faceNormal,
        // Unity's mesh.normals/tangents/colors may be absent. The has* flags are the contract:
        // each array is only indexed when its flag confirms a present, correctly sized channel.
        ...
        List<Vector2> uv)
    {
        ...
        vertices.Add(vertex);
        // Each has* flag was computed as "array is non-null and matches the vertex count", so the
        // array is present wherever its flag is true.
