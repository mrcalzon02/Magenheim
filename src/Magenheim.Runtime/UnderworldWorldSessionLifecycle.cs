...
/// layer state cannot leak into the next loaded world.
/// </summary>
internal sealed class UnderworldWorldSessionLifecycle : MonoBehaviour
{
    ...
    internal void Configure(UnderworldRuntimeServices services, ManualLogSource log)
    {
        if (_services is not null)
            throw new InvalidOperationException("Underworld world-session lifecycle is already configured.");
    ...
    private void Update()
    {
        ...
        if (znet is null)
        {
            if (_observedWorldUid.HasValue)
            {
                ...
                _log?.LogDebug($"Underworld logical context reset after Valheim world {_observedWorldUid.Value} unloaded.");
        ...
        if (_observedWorldUid.Value == currentWorldUid) return;

        var previous = _observedWorldUid.Value;
        _services.ResetForWorldUnload();
        _observedWorldUid = currentWorldUid;
        _log?.LogInfo($"Underworld logical context reset for Valheim world change {previous} -> {currentWorldUid}.");
    }
