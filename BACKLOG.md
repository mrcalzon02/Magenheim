...
Preserve the reusable authorities: six-biome terrain rules, deterministic layout, progression, ecology and Conclave content. The retired custom map projection/raster/exploration engine is **not** reusable authority: Overworld and Underworld now keep separate vanilla Minimap payloads/textures behind the same Valheim Minimap calls so ordinary minimap mods can reach both layers through their normal hooks. The per-player transition transaction/recovery subsystem is superseded and quarantined; only a thin Deep Gate transport adapter may replace it.
...
- [x] **Crystal weapons still carry generated placeholder textures.** DONE 0.0.98 (static; live look pending). Each weapon now carries its own atlas baked in Blender from the same `.blend` it exports from -- occlusion, worn edges and a per-material pattern -- replacing the seven shared Pillow noise maps. See `docs/validation/2026-09-22-crystal-weapon-readability-pass.md`. Original note: Reported against the staff
...
- [x] **Weapons are the lowest-detail family in the library.** DONE 0.0.98 (static; live look pending). Re-authored from designed forms by `tools/author-crystal-weapons.py`: edged blades with fullers or midribs, swept guards, banded grip wraps, wedged axe heads, flanged mace, a bow whose string no longer passes through its limbs. Median rose 1,206 -> 1,810 triangles (measured from the committed catalog); that is still under the library median, deliberately -- the detail went where it changes the silhouette, which vanilla weapons of 1-2k triangles also do, not into count. Original note: Median 1,262 triangles against a library median of 2,348 and a staff median of 2,316 — roughly half the library standard, on the assets held closest to camera. This is the same defect shape as the crystal progression items under P0.1 (lowest-poly assets despite highest player contact) and needs the same treatment: re-author to the library standard with silhouette detail that survives at held distance.
...
- [ ] U2: prove dedicated instance admission/materialization in a disposable environment before biome production. **LIVE DEFECT 2026-09-21:** the first native materializer rendered instance-local X/Y/Z directly into Unity world coordinates, so the Underworld appeared on top of Meadows terrain instead of occupying an isolated internal layer. Source correction now backs instance index 1 at an opaque engine Y layer, converts engine/local coordinates only at the runtime boundary, and applies Dungeon-Splitter-style ZDO layer filtering for load/sync discovery. This remains open until a rebuilt live Surface -> Deep Gate -> Underworld -> Surface run proves no Surface overlap, no cross-layer ZDO visibility/ownership, and correct native Underworld map/environment behavior.
- [ ] U3: harden instance persistence and multiplayer instance synchronization. **Map adapter source implemented 2026-09-21:** the large map owns Overworld/Underworld tabs on the single Valheim `Minimap`; the small map follows the physical layer; each layer carries a separate native Minimap payload and texture set; ordinary `UpdateExplore`/`Explore`, fog, saved pins, zoom/input/rendering and serialization remain Valheim-owned; dynamic physical-world pins are suppressed while browsing the other layer; repeated payload swaps reset vanilla's private transient-pin caches; and Underworld `GenerateWorldMap` sampling carries an immutable layer context into `Task.Run` workers used by asynchronous map-generation mods. The old custom map projection/raster files and tests were deleted. Live acceptance remains required for tab switching, reveal persistence, saved/shared pins, async map generators, map-size mods, and Surface/Underworld isolation. Do not build persistent player-transition state.
